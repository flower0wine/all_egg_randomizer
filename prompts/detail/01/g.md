...
- 目前大部分工作已经完成，但是还存在一些还未实现的功能，玩家回答超时的那个事件还存在问题，预期是实现三个随机效果，立即处死、丢到天空最高处落地、255级漂浮效果。src\main\java\com\alleggrandomizer\core\generator\event\MathQuizEvent.java。

# 用户需求

- 阅读 src\main\java\com\alleggrandomizer\core\EggHitHandler.java，这个包含了目前的砸鸡蛋的逻辑。
- 阅读生成事件的相关的逻辑，现在需要添加一个新的事件，鸡蛋砸中的地方生成一个激活的tnt，砸中的地方生成一群蝙蝠并有个女巫，砸中的地方生成一个末地水晶 末地水晶正上方6格生成一个下降的箭矢使他被打爆，砸中的地方生成6-12只彩虹羊，砸中的地方生成1只有马鞍的猪，砸中的地方生成倒着走路的彩虹羊，砸中的地方生成tnt矿车，再5S内回答随机一个100以内加减法 给予该玩家回答超时惩罚（立即处死 丢到天空最高处落地 255漂浮效果 ）回答正确随机给予绿宝石、钻石、下届合金锭2-6个 玩家需要在聊天栏打出正确答案。
