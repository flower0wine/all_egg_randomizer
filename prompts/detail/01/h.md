...
- 目前大部分工作已经完成，但是还存在一些还未实现的功能，src\main\java\com\alleggrandomizer\core\generator\event\ReverseRainbowSheepEvent.java，src\main\java\com\alleggrandomizer\core\generator\event\RainbowSheepEvent.java。
- 还有倒着走路指的是羊的模型上下颠倒，目前在研究如果将服务端数据传递给客户端，因为目前服务端需要标记羊是否需要倒立，客户端进行渲染。
- 但是目前存在 build 错误，因为 API 不存在，现在弄不清楚在这个版本当中该怎么传递数据给客户端。
...
- 阅读生成事件的相关的逻辑，现在需要添加一个新的事件，砸中的地方生成6-12只彩虹羊，砸中的地方生成倒着走路的彩虹羊。
